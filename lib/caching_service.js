...
exports.pipe_caching_service = function pipe_caching_service(options){
    ...
    return function pipe_caching_service(req,res,next){
        ...
        res.on('pipe',function(src){
            console.log('handling res pipe event from ' + src);
            console.log(fstream.writable ? 'writable':'fstream is not');
            console.log(res.readable ? 'readable':'res is not');
            console.log(src.readable ? 'readable':'src is not');
            src.pipe(fstream);
        });
        makedir(targetpath,function(){
            ...
            //     var rfstream = fs.createReadStream(filename);
            //     rfstream.pipe(res);
            });
            next();
        });
