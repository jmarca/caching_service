...
function appendGeoJSON(options,cb){
    var fstream = fs.createWriteStream(options.file);
    function writer(){
        options.writer(arguments);
        if(fstream.writable) {
            fstream.write(arguments);
        }else{
            fstream.destroy();
            cb(new Error('fs buffer error'));
        }

    };
    writer.end = function(){
        fstream.end(arguments);
        fstream.destroySoon();
    }
    return writer;
};
...
exports.file_caching_service = function file_caching_service(options){
    ...
    return function file_caching_service(req,res,next){
        ...
        makedir(targetpath,function(){
            var write = appendGeoJSON({file:filename,writer:oldwrite},next);
            res.write = write;
            res.end = function(){
                res.end = oldend;
                write.end(arguments);
