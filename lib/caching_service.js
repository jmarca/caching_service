...
module.exports = function caching_service(options,wrapped){
    ...
    var banana = 0;
    return function caching_service(req,res,next){
        var oldend = res.end;

        var targetpath = getPath(req);
        var filename = [targetpath,getFile(req)].join('/');
        console.log('in the tile cacher with '+filename);
        var localWriteEnd = function(doc){
            var writeOut = writeGeoJSON({file:filename,doc:doc});
            makeParentDir(targetpath,writeOut);
        };

        console.log('replacing the end of the response');
        //res.end=replaceEnd(res,filename,localWriteEnd,myend);


        // I can't figure out why, but using res.end fails here
        res.e=function(chunk,encoding){
            res.end = oldend;
